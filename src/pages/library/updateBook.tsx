import { putBookImage, type putBookImageTypeRequest } from "../../http/putBookImage"
import { deleteBook, type deleteBookTypeRequest } from "../../http/deleteBook"
...
export function UpdateBook(){
    ...
    const bookImage = useMutation<void, Error, putBookImageTypeRequest>({
        mutationFn: putBookImage,
        onSuccess: () => {
    ...
    const bookDelete = useMutation<void, Error, deleteBookTypeRequest>({
                            ...
                            <form onSubmit={handleSubmit(formBookPut)} className="flex gap-6">
                                <div className="flex-1/5 flex flex-col gap-2" > 
                                    ...
                                    <div className="bg-font-500 p-6 py-6 border-1 border-font-200 rounded-lg flex flex-col gap-2" >
                                        <InputSelect valueDefault={dataBook.category} {...register("category")} label="Categoria/Genero" options={categories} />
                                        <InputSelect valueDefault={dataBook.edition} {...register("edition")} label="Edição" options={edition} />
                                        <InputSelect valueDefault={dataBook.year_publi} {...register("year_publi")} label="Ano de publicação" options={yearOfPublication} />
                                        <InputSelect valueDefault={dataBook.finishing} {...register("finishing")} label="Acabamento" options={finish} />
                                ...
                                <div className="bg-font-500 flex-2/3 px-8 py-3 w-full flex flex-col gap-3  border-1 border-font-200 rounded-lg" >
                                    ...
                                    <div className="flex justify-between items-center mt-2">
                                        <span className="flex items-center gap-2">
                                            <input onChange={handleCheckBox} className="w-5 h-5" type="checkbox" id="checkbox" checked={valueCheckBox}/>
                                        ...
                                        <div className="flex gap-8">
                                            <SmallButton 
                                                ...
                                                text="Excluir" 
                                                isSave={false} 
                                                onClick={() => handleDeleteBook(param.id!)}
                                            />
                                            <SmallButton 
                                                type="submit" 
                                                text="Salva" 
